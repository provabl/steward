...
const amKeyID = "steward-am-ephemeral"
...
//
// The key is ephemeral on purpose, exactly as in vet: steward's appraisal is
...
// spine check needs. The durable artifact is the lowered gate-result.json, never
// the evidence bundle, so nothing reads this signature after the process exits. A
// persisted key would add a key-management surface for no benefit and would weaken
// freshness (it would let a stale bundle from a prior run verify — the exact thing
// the nonce+SIG spine prevents). A named/persisted key earns its place only when
// evidence is appraised out-of-process — a non-goal here.
...
func NewEphemeralAM() (*EphemeralAM, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("steward: generate AM key: %w", err)
	}
...
// Root implements trust.Store. steward brings no external trust roots (provenance
// is recorded-fact evidence; integrity is the appraiser's recompute concern), so
// there are none to resolve here.
